...
As of `2026.4.30.2`, the ResponseRay backend parses macOS raw artifacts in-process (rather than just hashing/preserving them on the timeline). The native parser lives at `backend/internal/collectoringest/macos/` and runs automatically when an upload's manifest declares `"platform": "macos"`.
